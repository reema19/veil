...
struct AudioRecorderView: View {
        ...
        VStack(spacing: 12) {
            ZStack {
                if viewModel.state == .recording {
                    AudioBlob(size: blobSize * 1.55, opacity: 0.10, level: viewModel.soundLevel)
                    AudioBlob(size: blobSize * 1.28, opacity: 0.18, level: viewModel.soundLevel * 0.9)
                    AudioBlob(size: blobSize, opacity: 1.0, level: viewModel.soundLevel)
                } else {
                    OrganicAudioBlob()
                        .fill(Color.white)
                ...
                if viewModel.state == .recording {
                    ...
                    OrganicAudioBlob()
                        ...
                        .frame(width: blobSize, height: blobSize)
                        .animation(.linear(duration: 0.2), value: viewModel.recordingProgress)
                }
                ...
                HStack(spacing: 5.5) {
                    ForEach(Array(samples.enumerated()), id: \.offset) { _, sample in
                        RoundedRectangle(cornerRadius: 2)
                            ...
                            .frame(width: 3.5, height: max(6, sample * 52))
                    }
    ...
    private var normalizedWaveformSamples: [CGFloat] {
        ...
        return viewModel.waveformSamples.prefix(18).map { sample in
